...
export function ReviewProfileModal({ t, action, onClose, onConfirm }: ReviewProfileModalProps) {
      ...
      <div className="bg-white rounded-[12px] shadow-lg w-full max-w-[500px] overflow-hidden animate-in fade-in zoom-in duration-200">
        <div className="px-6 py-4 border-b border-[#E7E8EB]">
          <h3 className="text-[18px] font-bold text-[#0A1B39]">
            {action === "approve" ? (t.clinic?.approve_profile || "Approve Profile") : (t.clinic?.reject_profile || "Reject Profile")}
          </h3>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 flex flex-col gap-5">
          ...
          <p className="text-[14px] text-[#6C7688]">
            {action === "approve"
              ? "Are you sure you want to approve this doctor's entire profile?"
              : "Please provide a reason for rejecting this doctor's profile."
            }
          </p>

          <div className="flex flex-col gap-2">
            <label className="text-[13px] font-bold text-[#0A1B39]">
              {t.clinic?.action_reason || "Reason"}
              {action === "reject" && <span className="text-[#EF1E1E] ml-1">*</span>}
            </label>
            <textarea
              value={reason}
              ...
              placeholder={action === "reject" ? (t.clinic?.reason_required || "Reason (10-500 char)") : "Optional reason"}
              maxLength={500}
            />
          ...
          <div className="flex items-center justify-end gap-3 pt-2">
            ...
            <button
              ...
              className={`px-5 py-2 text-white text-[14px] font-medium rounded-[8px] transition-colors disabled:opacity-50 ${action === "approve" ? "bg-[#27AE60] hover:bg-[#219653]" : "bg-[#EF1E1E] hover:bg-[#DC2626]"}`}
            >
