...
import { SkinDisease, ContentPagination, ContentStatistics } from "@/types/admin-content";
...
export default function SkinDiseasesView({ t }: SkinDiseasesViewProps) {
  ...
  const fetchStats = useCallback(async () => {
    try {
      ...
      if (res.success && res.data?.skin_diseases) {
        setStats(res.data.skin_diseases);
      }
    } catch {
      // silently fail
    } finally {
  ...
  // Documented params: page, limit, is_active
  const fetchDiseases = useCallback(async (page: number, filter: FilterTab) => {
    try {
      setIsLoading(true);
      setError(null);
      ...
      const res = await getSkinDiseases(params);
      if (res.success) {
        setDiseases(Array.isArray(res.data) ? res.data : (res.data && Array.isArray((res.data as any).data) ? (res.data as any).data : []));
        setPagination(res.pagination || { page: 1, limit: 10, total: 0, pages: 0 });
      } else {
        setError(res.message || "Failed to load skin diseases");
      }
    } catch (err: any) {
  ...
  useEffect(() => {
    fetchDiseases(1, activeFilter);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [activeFilter]);

  useEffect(() => {
    fetchDiseases(pagination.page, activeFilter);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [pagination.page]);
  ...
  const handleToggleStatus = async (id: number, currentStatus: boolean) => {
    try {
      ...
      if (res.success) {
        setDiseases(prev => prev.map(d => d.id === id ? { ...d, is_active: !currentStatus } : d));
        setStats(prev => {
          if (!prev) return prev;
          return {
            ...prev,
  ...
  const filterTabs: { key: FilterTab; label: string; activeClass: string }[] = [
    { key: "all", label: t.health_tips?.filter_all || "All", activeClass: "bg-[#9B59B6] text-white" },
    ...
    <div className="flex flex-col gap-6 w-full p-4 sm:p-6 bg-[#F5F6F8] min-h-screen">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 w-full bg-white p-5 border border-[#E7E8EB] rounded-[14px] shadow-sm animate-in fade-in slide-in-from-top-4 duration-500">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 flex items-center justify-center rounded-[10px] bg-gradient-to-br from-[#9B59B6] to-[#C07FD9] text-white shadow-md shadow-purple-200">
            <SkinIcon />
          </div>
          <div>
            <h2 className="text-[20px] sm:text-[22px] font-bold text-[#0A1B39]">
              {t.health_tips.skin_diseases_title}
            </h2>
            <p className="text-[12px] text-[#6C7688] mt-0.5">
              {t.health_tips?.manage_diseases || "Manage skin diseases information"}
            </p>
          </div>
          <span className="px-3 py-1 bg-[#F3E8FF]/60 text-[#9B59B6] text-[12px] font-bold rounded-[6px] border border-[#F3E8FF]">
            {pagination.total}
          </span>
        </div>
        ...
        <button
          ...
          className="w-full sm:w-auto flex items-center justify-center gap-2 px-5 py-2.5 bg-gradient-to-r from-[#9B59B6] to-[#C07FD9] text-white rounded-[10px] text-[13px] font-semibold hover:shadow-lg hover:shadow-purple-200 hover:-translate-y-0.5 transition-all duration-200 shadow-sm shrink-0"
        >
        ...
        {[
          {
            ...
            icon: <TotalIcon />,
            gradient: "from-[#9B59B6] to-[#C07FD9]",
            text: "text-[#9B59B6]",
          },
          {
      ...
      {error && (
        <div className="bg-[#FEF2F2] border border-[#EF1E1E]/30 text-[#EF1E1E] p-4 rounded-[12px] flex items-center justify-between">
          ...
          <button
            onClick={() => fetchDiseases(pagination.page, activeFilter)}
          ...
          <table className="w-full min-w-[1050px]">
            <thead className="bg-[#FAFBFC]">
              <tr className="border-b border-[#E7E8EB]">
                <th className="py-4 px-6 text-start text-[12px] font-bold text-[#6C7688] uppercase tracking-wide w-[24%]">
                  {t.health_tips.title_en}
                </th>
                <th className="py-4 px-6 text-start text-[12px] font-bold text-[#6C7688] uppercase tracking-wide w-[24%]">
                  {t.health_tips.title_ar}
                </th>
                <th className="py-4 px-5 text-center text-[12px] font-bold text-[#6C7688] uppercase tracking-wide w-[14%]">
                  {t.health_tips.website_link || "Reference Link"}
                </th>
                <th className="py-4 px-5 text-start text-[12px] font-bold text-[#6C7688] uppercase tracking-wide w-[16%]">
                  {t.health_tips.date_added}
                </th>
                <th className="py-4 px-5 text-start text-[12px] font-bold text-[#6C7688] uppercase tracking-wide w-[14%]">
            ...
            <tbody>
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={6} className="py-16 text-center">
                    <div className="flex flex-col items-center gap-3">
                      <div className="w-14 h-14 flex items-center justify-center rounded-full bg-[#F3E8FF] text-[#9B59B6]">
                        <SkinIcon />
                      </div>
                      ...
                      <p className="text-[12px] text-[#9DA4B0]">
                        {t.health_tips?.no_diseases_hint || "Add your first skin disease entry using the button above."}
                      </p>
                    </div>
                  </td>
                </tr>
                  ...
                  >
                    ...
                    <td className="py-4 px-6">
                      <div className="flex flex-col gap-0.5">
                        <span className="text-[14px] font-semibold text-[#0A1B39] group-hover:text-[#9B59B6] transition-colors line-clamp-1">
                          {disease.title_en || <span className="text-[#9DA4B0] font-normal italic text-[13px]">No English title</span>}
                        </span>
                    ...
                    <td className="py-4 px-5 text-center">
                      {disease.website_link ? (
                        <a
                          ...
                          className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-[#EFF6FF] text-[#2F80ED] hover:bg-[#2F80ED] hover:text-white transition-all duration-150 text-[11px] font-bold rounded-[6px] border border-[#2F80ED]/20 group/link"
                          title={disease.website_link}
                        >
                          <ExternalLinkIcon />
                          {t.health_tips.visit_link}
                        </a>
                      ) : (
                        <span className="text-[12px] text-[#9DA4B0] italic">No link</span>
                      )}
                    </td>
                    ...
                    <td className="py-4 px-5">
                      <span className="text-[13px] text-[#6C7688]" dir="ltr">
                        {disease.created_at
                          ? new Date(disease.created_at).toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" })
                          : "—"}
                      </span>
                    </td>
                    ...
                    <td className="py-4 px-5">
                      <label
                        className={cn(
                          ...
                          actionLoadingId === disease.id && "opacity-50 pointer-events-none"
                        )}
                      >
                        <div className="relative">
                          <input
                            ...
                            onChange={() => handleToggleStatus(disease.id, disease.is_active)}
                          />
                        ...
                        <span className={cn("text-[11px] font-bold px-2 py-0.5 rounded-[5px]", disease.is_active ? "text-[#27AE60] bg-[#E8F8EE]" : "text-[#9DA4B0] bg-[#F3F4F6]")}>
                    ...
                    <td className="py-4 px-6 text-end">
                      <CustomDropdown
                        trigger={
                          <button
                            className="w-8 h-8 flex items-center justify-center rounded-[8px] border border-[#E7E8EB] text-[#6C7688] hover:border-[#9B59B6] hover:text-[#9B59B6] hover:bg-[#F3E8FF]/40 transition-all duration-150 disabled:opacity-40"
                            disabled={actionLoadingId === disease.id}
                          >
                            {actionLoadingId === disease.id ? (
                              <div className="w-3.5 h-3.5 border-2 border-[#9B59B6] border-t-transparent rounded-full animate-spin" />
                            ) : (
                              <MoreVerticalIcon />
                            )}
                          </button>
                        ...
                        items={[
                          { label: t.health_tips.edit_disease, onClick: () => handleEdit(disease) },
                          {
                            ...
                            onClick: () => handleDelete(disease.id, disease.title_en || disease.title_ar),
          ...
          <div className="p-4 border-t border-[#E7E8EB] flex items-center justify-between bg-[#FAFBFC]">
            ...
            <div className="flex items-center gap-1.5">
              <button
                ...
                className="px-3 py-1.5 text-[12px] font-medium border border-[#E7E8EB] rounded-[6px] text-[#0A1B39] disabled:opacity-40 hover:bg-[#F3E8FF]/40 hover:border-[#9B59B6] hover:text-[#9B59B6] transition-all"
              >
