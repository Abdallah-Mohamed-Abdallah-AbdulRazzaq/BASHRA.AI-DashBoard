  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload: MedicalArticlePayload = {
        ...
        sub_title_ar: formData.sub_title_ar || undefined,
        sub_title_en: formData.sub_title_en || undefined,
      ...
      if (res.success) {
        ...
        setError(res.message || t.common?.error || "Failed to save article.");
      }
    } catch (err: any) {
        ...
        <form onSubmit={handleSubmit} className="p-6 flex flex-col gap-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input type="text" value={formData.title_en || ""} onChange={e => setFormData({...formData, title_en: e.target.value})} className="w-full h-10 px-3 bg-white border border-[#E7E8EB] rounded-[8px] text-[13px] text-[#0A1B39] focus:outline-none focus:border-[#2E37A4] transition-colors disabled:opacity-50" placeholder="e.g. Skin Care Basics" dir="ltr" disabled={isSubmitting} />
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input type="text" required value={formData.title_ar} onChange={e => setFormData({...formData, title_ar: e.target.value})} className="w-full h-10 px-3 bg-white border border-[#E7E8EB] rounded-[8px] text-[13px] text-[#0A1B39] focus:outline-none focus:border-[#2E37A4] transition-colors disabled:opacity-50" placeholder="مثال: أساسيات العناية بالبشرة" dir="rtl" disabled={isSubmitting} />
            </div>

            {/* English Sub Title (Optional) */}
            <div className="flex flex-col gap-1.5">
              <label className="text-[13px] font-semibold text-[#0A1B39]">{t.health_tips.sub_title_en}</label>
              <input type="text" value={formData.sub_title_en || ""} onChange={e => setFormData({...formData, sub_title_en: e.target.value})} className="w-full h-10 px-3 bg-white border border-[#E7E8EB] rounded-[8px] text-[13px] text-[#0A1B39] focus:outline-none focus:border-[#2E37A4] transition-colors disabled:opacity-50" placeholder="e.g. Daily routines" dir="ltr" disabled={isSubmitting} />
            </div>

            {/* Arabic Sub Title (Optional) */}
            <div className="flex flex-col gap-1.5">
              <label className="text-[13px] font-semibold text-[#0A1B39]">{t.health_tips.sub_title_ar}</label>
              <input type="text" value={formData.sub_title_ar || ""} onChange={e => setFormData({...formData, sub_title_ar: e.target.value})} className="w-full h-10 px-3 bg-white border border-[#E7E8EB] rounded-[8px] text-[13px] text-[#0A1B39] focus:outline-none focus:border-[#2E37A4] transition-colors disabled:opacity-50" placeholder="مثال: روتين يومي" dir="rtl" disabled={isSubmitting} />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-5">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <textarea value={formData.description_en || ""} onChange={e => setFormData({...formData, description_en: e.target.value})} className="w-full min-h-[120px] p-3 bg-white border border-[#E7E8EB] rounded-[8px] text-[13px] text-[#0A1B39] focus:outline-none focus:border-[#2E37A4] transition-colors resize-y custom-scrollbar disabled:opacity-50" placeholder="Enter article content..." dir="ltr" disabled={isSubmitting} />
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <textarea required value={formData.description_ar} onChange={e => setFormData({...formData, description_ar: e.target.value})} className="w-full min-h-[120px] p-3 bg-white border border-[#E7E8EB] rounded-[8px] text-[13px] text-[#0A1B39] focus:outline-none focus:border-[#2E37A4] transition-colors resize-y custom-scrollbar disabled:opacity-50" placeholder="أدخل محتوى المقال..." dir="rtl" disabled={isSubmitting} />
            </div>
          </div>
