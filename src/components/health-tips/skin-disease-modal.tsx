...
export const SkinDiseaseModal = ({ isOpen, onClose, t, editData }: SkinDiseaseModalProps) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    console.log("Submitting Disease Data:", formData);
    onClose();
  };
        ...
        <form onSubmit={handleSubmit} className="p-6 flex flex-col gap-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <div className="flex flex-col gap-1.5">
              ...
              <input type="text" required value={formData.title_en} onChange={e => setFormData({...formData, title_en: e.target.value})} className="w-full h-10 px-3 bg-white border border-[#E7E8EB] rounded-[8px] text-[13px] text-[#0A1B39] focus:outline-none focus:border-[#2E37A4] transition-colors" placeholder="e.g. Psoriasis" dir="ltr" />
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-[13px] font-semibold text-[#0A1B39]">{t.health_tips.title_ar} <span className="text-[#EF1E1E]">*</span></label>
              <input type="text" required value={formData.title_ar} onChange={e => setFormData({...formData, title_ar: e.target.value})} className="w-full h-10 px-3 bg-white border border-[#E7E8EB] rounded-[8px] text-[13px] text-[#0A1B39] focus:outline-none focus:border-[#2E37A4] transition-colors" placeholder="مثال: الصدفية" dir="rtl" />
            </div>
          </div>

          {/* Website Link (Optional based on SQL) */}
          <div className="flex flex-col gap-1.5">
            ...
            <div className="relative">
              <input 
                ...
                className="w-full h-10 pl-10 pr-4 rtl:pr-10 rtl:pl-4 bg-white border border-[#E7E8EB] rounded-[8px] text-[13px] text-[#0A1B39] focus:outline-none focus:border-[#2E37A4] transition-colors" 
                placeholder="https://www.medical-reference.com/disease" 
                dir="ltr" 
              />
          ...
          <div className="flex justify-end gap-3 pt-5 border-t border-[#E7E8EB]">
            ...
            <button type="submit" className="px-6 py-2.5 bg-[#2E37A4] text-white text-[13px] font-medium rounded-[8px] hover:bg-[#252D88] transition-colors shadow-sm">
              {isEditing ? "Save Changes" : "Create Info"}
            </button>
          </div>
