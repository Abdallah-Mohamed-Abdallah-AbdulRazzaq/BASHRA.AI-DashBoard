...
import { createSkinDisease, updateSkinDisease } from "@/lib/admin-content";
import { SkinDisease, SkinDiseasePayload } from "@/types/admin-content";
import { getApiErrorMessage } from "@/lib/error-utils";

interface SkinDiseaseModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
  t: any;
  editData?: SkinDisease | null;
}
...
export const SkinDiseaseModal = ({ isOpen, onClose, onSuccess, t, editData }: SkinDiseaseModalProps) => {
  const [formData, setFormData] = useState<SkinDiseasePayload>({
    title_en: "",
    title_ar: "",
    description_en: "",
    description_ar: "",
  ...
  const modalTitle = isEditing ? t.health_tips.edit_disease : t.health_tips.add_new_disease;
  ...
  const validate = (): boolean => {
    ...
    if (formData.website_link) {
      try {
        new URL(formData.website_link);
      } catch {
        newErrors.website_link = "Must be a valid URL (e.g. https://example.com).";
      }
    }
    setErrors(newErrors);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload: SkinDiseasePayload = {
      ...
      if (isEditing && editData?.id) {
        res = await updateSkinDisease(editData.id, payload);
      } else {
        res = await createSkinDisease(payload);
      }

      if (res.success) {
        onSuccess();
      } else {
        setApiError(res.message || t.common?.error || "Failed to save skin disease info.");
      }
    } catch (err: any) {
  ...
  const inputNormal = cn(inputBase, "border-[#E7E8EB] focus:border-[#9B59B6] focus:ring-2 focus:ring-[#9B59B6]/10");
  ...
  const SectionLabel = ({ label }: { label: string }) => (
    <div className="flex items-center gap-2">
      <div className="w-1 h-4 rounded-full bg-gradient-to-b from-[#9B59B6] to-[#C07FD9]" />
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="bg-white rounded-[18px] shadow-2xl w-full max-w-[700px] max-h-[92vh] overflow-y-auto z-10 relative flex flex-col animate-in zoom-in-95 slide-in-from-bottom-4 duration-300">

        {/* Header */}
        <div className="px-6 py-5 border-b border-[#E7E8EB] bg-gradient-to-r from-[#FAFBFC] to-[#FDF4FF] sticky top-0 z-20 flex justify-between items-center rounded-t-[18px]">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 flex items-center justify-center rounded-[8px] bg-gradient-to-br from-[#9B59B6] to-[#C07FD9] text-white shadow-sm">
        ...
        <form onSubmit={handleSubmit} className="p-6 flex flex-col gap-5">
          ...
          {/* Reference Link */}
          <SectionLabel label="Reference / المرجع" />
          <div className="flex flex-col gap-1">
            <label className="text-[12px] font-semibold text-[#0A1B39]">
              {t.health_tips.website_link || "Website Link"}
              <span className="ml-1 text-[10px] font-normal text-[#9DA4B0]">(Optional)</span>
            </label>
            <div className="relative">
              <div className="absolute inset-y-0 start-0 flex items-center ps-3.5 pointer-events-none text-[#9DA4B0]">
              ...
              <input
                ...
                className={cn(errors.website_link ? inputError : inputNormal, "ps-10")}
                placeholder="https://www.example.com/disease-info"
                dir="ltr"
                disabled={isSubmitting}
              />
            </div>
            <FieldError field="website_link" />
          </div>
          ...
          <div className="grid grid-cols-1 gap-4">
            ...
            <div className="flex flex-col gap-1">
              ...
              <textarea
                ...
                className={cn(textareaBase, "min-h-[100px]", errors.description_en ? "border-[#EF1E1E] focus:ring-2 focus:ring-[#EF1E1E]/10" : "border-[#E7E8EB] focus:border-[#9B59B6] focus:ring-2 focus:ring-[#9B59B6]/10")}
            ...
            <div className="flex flex-col gap-1">
              ...
              <textarea
                ...
                className={cn(textareaBase, "min-h-[100px]", errors.description_ar ? "border-[#EF1E1E] focus:ring-2 focus:ring-[#EF1E1E]/10" : "border-[#E7E8EB] focus:border-[#9B59B6] focus:ring-2 focus:ring-[#9B59B6]/10")}
                placeholder="أدخل المعلومات بالعربية..."
                dir="rtl"
                disabled={isSubmitting}
              />
              <div className="flex justify-between">
          ...
          <div className="flex items-center justify-between p-4 bg-gradient-to-r from-[#F5F6F8] to-[#ECEDF0] rounded-[12px] border border-[#E7E8EB]">
            <div>
              ...
              <p className="text-[11px] text-[#9DA4B0] mt-0.5">
                {formData.is_active ? "This disease info will be visible to users" : "This disease info will be hidden from users"}
              </p>
            </div>
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-[#E7E8EB]">
            ...
            <button
              type="submit"
              disabled={isSubmitting}
              className="px-7 py-2.5 bg-gradient-to-r from-[#9B59B6] to-[#C07FD9] text-white text-[13px] font-semibold rounded-[10px] hover:shadow-lg hover:shadow-purple-200 hover:-translate-y-0.5 transition-all duration-200 disabled:opacity-50 disabled:hover:translate-y-0 flex items-center gap-2"
            >
