...
export const SkinDiseaseModal = ({ isOpen, onClose, onSuccess, t, editData }: SkinDiseaseModalProps) => {
  ...
  useEffect(() => {
    if (editData && isOpen) {
      ...
      setFormData({ title_en: "", title_ar: "", description_en: "", description_ar: "", website_link: "", is_active: true });
      setError(null);
    }
  }, [editData, isOpen]);
  ...
  const title = isEditing ? t.health_tips.edit_disease : t.health_tips.add_new_disease;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        setError(res.message || t.common?.error || "Failed to save disease.");
      }
    } catch (err: any) {
        ...
        <form onSubmit={handleSubmit} className="p-6 flex flex-col gap-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input type="text" value={formData.title_en || ""} onChange={e => setFormData({...formData, title_en: e.target.value})} className="w-full h-10 px-3 bg-white border border-[#E7E8EB] rounded-[8px] text-[13px] text-[#0A1B39] focus:outline-none focus:border-[#2E37A4] transition-colors disabled:opacity-50" placeholder="e.g. Eczema" dir="ltr" disabled={isSubmitting} />
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input type="text" required value={formData.title_ar} onChange={e => setFormData({...formData, title_ar: e.target.value})} className="w-full h-10 px-3 bg-white border border-[#E7E8EB] rounded-[8px] text-[13px] text-[#0A1B39] focus:outline-none focus:border-[#2E37A4] transition-colors disabled:opacity-50" placeholder="مثال: الأكزيما" dir="rtl" disabled={isSubmitting} />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-5">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <textarea value={formData.description_en || ""} onChange={e => setFormData({...formData, description_en: e.target.value})} className="w-full min-h-[100px] p-3 bg-white border border-[#E7E8EB] rounded-[8px] text-[13px] text-[#0A1B39] focus:outline-none focus:border-[#2E37A4] transition-colors resize-y custom-scrollbar disabled:opacity-50" placeholder="Enter description..." dir="ltr" disabled={isSubmitting} />
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <textarea required value={formData.description_ar} onChange={e => setFormData({...formData, description_ar: e.target.value})} className="w-full min-h-[100px] p-3 bg-white border border-[#E7E8EB] rounded-[8px] text-[13px] text-[#0A1B39] focus:outline-none focus:border-[#2E37A4] transition-colors resize-y custom-scrollbar disabled:opacity-50" placeholder="أدخل الوصف..." dir="rtl" disabled={isSubmitting} />
            </div>
          </div>

          {/* Website Link */}
          <div className="flex flex-col gap-1.5">
            <label className="text-[13px] font-semibold text-[#0A1B39]">{t.health_tips.website_link}</label>
            <input type="url" value={formData.website_link || ""} onChange={e => setFormData({...formData, website_link: e.target.value})} className="w-full h-10 px-3 bg-white border border-[#E7E8EB] rounded-[8px] text-[13px] text-[#0A1B39] focus:outline-none focus:border-[#2E37A4] transition-colors disabled:opacity-50" placeholder="https://example.com/eczema" dir="ltr" disabled={isSubmitting} />
          </div>
